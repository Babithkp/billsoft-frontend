...
export default function InventryPage({ data }: { data: ItemInputs[] }) {
  ...
  const [filteredPurchase, setFilteredPurchase] = useState<PurchaseInputs[]>(
    [],
  );
  ...
  useEffect(() => {
    const delay = setTimeout(() => {
      const text = purchaseSearch.trim().toLowerCase();
  ...
  const setPurchaseValuesToInputs = (purchase: PurchaseInputs) => {
    setFormStatus("edit");
    setValuePurchase("id", purchase.id);

    setValuePurchase("itemName", purchase.item.id);
    setValuePurchase(
      "date",
      new Date(purchase.date).toISOString().split("T")[0],
    );
    setValuePurchase("purchasePrice", purchase.purchasePrice);
    setValuePurchase("quantity", purchase.quantity);
    setValuePurchase("amount", purchase.amount);
    setValuePurchase("paymentType", purchase.paymentType);
    setValuePurchase("transactionId", purchase.transactionId);
  };
  ...
  const deletePurchase = async () => {
    ...
    const response = await deletePurchaseApi(selectedPurchase.id);
    if (response && response.status === 200) {
      toast.success("Purchase deleted successfully");
      setPurchaseDetailsModal(false);
      getAllPurchases();
    } else {
      toast.error("Purchase deletion failed");
    }
    setIsloading(false);
  };
  ...
  const onSubmitPurchase = async (data: PurchaseInputs) => {
    ...
    if (formStatus === "create") {
      ...
      const response = await updatePurchaseApi(data);
      if (response && response.status === 200) {
  ...
  return (
    <>
        ...
        <div className="flex gap-10">
          ...
          <div className="flex w-full rounded-xl border bg-[#FAFAFA] p-5 shadow-md">
            <div className="flex items-center gap-5">
              ...
              <div className="font-medium">
                <p className="text-muted">Total Item Quantity</p>
                <p className="text-xl text-[#007E3B]">
                  {itemData
            ...
            {section.purchase && (
              <>
                ...
                <Button
                  ...
                  variant={"outline"}
                  onClick={() => setSection({ item: true, purchase: false })}
                >
                  View Item
                </Button>
              </>
            ...
            {section.item && (
              <>
                ...
                <Button
                  className="border-primary rounded-2xl"
                  variant={"outline"}
                  onClick={() => setSection({ item: false, purchase: true })}
                >
                  View Purchase
                </Button>
              </>
                ...
                <div>
                  ...
                  >
                    <div className="w-[30%]">
                      ...
                      {isItemNameExists && (
                        <p className="text-red-500">Item name already exists</p>
                      )}
                    </div>
                    <div className="w-[30%]">
                      <div className="flex flex-col gap-2">
              ...
              {filteredPurchase.map((item) => (
                <tr
                  ...
                  onClick={() => [
                    ...
                    setPurchaseValuesToInputs(item),
      ...
      <Dialog
        ...
        onOpenChange={setPurchaseDetailsModal}
      >
        <DialogTrigger className="hidden"></DialogTrigger>
        <DialogContent className="min-w-6xl">
          <DialogHeader className="flex">
            <DialogTitle className="text-secondary flex justify-between">
              <span>{selectedPurchase?.item.itemName}</span>
              <div className="mr-10 flex gap-3">
                <button
                  className="cursor-pointer"
                  onClick={() => [
                    setPurchaseValuesToInputs(selectedPurchase!),
                    setPurchaseCreateModal(true),
                  ]}
                >
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription className="font-medium text-black">
                        Are you sure you want to remove this Purchase? This
          ...
          <div className="grid grid-cols-4 gap-5">
            <div className="flex items-center gap-5">
              ...
              <p>INR {selectedPurchase?.purchasePrice}</p>
            </div>
            <div className="flex items-center gap-5">
              <label className="font-medium">Purchase Qty</label>
            ...
            <div className="col-span-2 flex items-center gap-5">
              ...
              <p>{selectedPurchase?.amount}</p>
            </div>
            <div className="flex items-center gap-5">
              <label className="font-medium">Purchase Date</label>
              <p>
                {new Date(
                  selectedPurchase?.date as string,
                ).toLocaleDateString()}
              </p>
            </div>
            <div className="flex items-start gap-2">
              <label className="font-medium">Payment type</label>
              <p>{selectedPurchase?.paymentType}</p>
            </div>
            <div className="flex items-center gap-5">
              <label className="text-sm font-medium">
                Transaction Reference Number
              </label>
              <p>{selectedPurchase?.transactionId}</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
